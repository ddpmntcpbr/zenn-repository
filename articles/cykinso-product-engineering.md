...
キャリア3年目のジュニアエンジニアが**プロダクトエンジニアリング**に興味を持ち、その第一歩目を企業の中で実践した体験記をシェアするものです。

## ハイライト

- プロダクトエンジニアリングとはXXである。
- 個人が実践できる第一歩目は、既存UXの体験・言語化である。
...
だからこそ、「未顧客」への定性的な理解を進めるためには、自分がまるっとUXを経験することが有意義と考えています
...
ここからは、具体的な実践の記録を書いていきます。なるべく、これを読んでくださっている方が応用できるような形で書き下していきます。

### UXのステップ分解

まずはじめに、弊社プロダクトが提供するUXを、いくつかのステップに分けてみるところから始めました。

特に意識したのは、最初にステップは「アプリを使い始めるところ」ではなく、「そもそもプロダクトを認知するところ」であるという点です。

以下の、弊社プロダクトである腸内細菌検査キットのUXステップを載せます。

|ステップ|詳細|
|---|---|
|認知|メディア、広告、口コミ、などを通じてプロダクトを認知する|
|購買|ECサイトでプロダクトを購入する|
|検査キットの到着・開封||
|検体の採取||
|アカウント登録||
...
（どんな情緒的価値を訴えてられたら、自分は2万円を払うだろうか）

### UX言語化② UI改善

UXステップごとにシートを作成し、感じたこと・改善できそうなことをリストに並べていきました。
...
を1~3でランクづけし、両者の和をリストアイテムの重要度としてソートしました。

### STEP2 自分をペルソナに見立てる

UI言語化をチーム内で共有し、改善タスクの選定を行いました。

...
先駆者である個人開発者何名かのSNSや開発ブログを調べたところ、以下の3点の共通点があることが分かりました。

**1. 王道ジャンルのニッチを攻める**
**2. サービスの利用が個人で完結している**
**3. 広告収入に頼らないマネタイズポイントを用意できている**
...
などです（ちなみにゲーム開発の場合は、脱出ゲームが無難に強そうでした）。

王道ジャンルのアプリは、それこそiOSであればApple製のアプリが最初からiOS端末に入っていたりするので、これを個人開発で超えるのは難しいから避けた方がいいのでは、と思っていました。

しかし、この考えは全くの逆であり、

「王道であるがゆえに見込みユーザーが多く、ちょっとした差別化でも十分にユーザー数の確保が期待できる。まだ世の中にない斬新なアプリは、誰も必要としてないから世の中にないのだ」

が真理なのだろう思いました。

### 2. サービスの利用が個人で完結している

例えばメルカリのようなECサイトや、XはIntagramのようなSNS、そしてzennやnoteのような記事投稿サイトなどの類は、「多くのユーザーが利用していること」がそのままサービスのクオリティに直結します。
...
対して、メモ帳アプリやタスク管理アプリのような、「サービスの利用が個人で完結する」ものであれば、全体の利用ユーザー数はサービスのクオリティと関係がありません。「ユーザーがユーザーを呼ぶ」流れにはなりにくいので爆発力はありませんが、「少人数のファンが使い続けてくれる」状態を維持しやすいのはメリットです。

月額500円の有料プランに100人が加入してくれれば、月5万円の収益になります。個人の副業収入と考えれば十分な金額でしょう。

また、運用にかかる時間的コストもほぼゼロである点も、個人開発者にとっては大きなメリットです。もちろんライブラリアップデートなどの対応はあると思いますが、例えば、ユーザーどうしのコミュニケーションに基づくトラブルへの対応などに自身のリソースが割かれるリスクはありません。

つまり、一度開発さえしてしまえば、この手のサービスが生み出す収益はかなり不労所得に近い収益として期待できる、ということです。

そして、 **「サービスの利用が個人で完結する」のであれば、Web アプリよりもネイティブアプリの自然なケースが多いだろう** と考えました。

zennのような「多くのユーザーが集まる公共の場所」を提供する手段としては Web アプリの方が優れていますが、「サービスの利用が個人で完結する」を前提に考えるなら、スマートフォンからアクセスできるネイティブアプリの方が、ユーザーの生活に根ざしやすいです。

実際、今回の事前調査の範囲においては、**収益化を成功しているアプリの多くがネイティブアプリとして作られていました**。

このポイントへの気づきが、**Web系の私があえてネイティブアプリの開発に挑戦した理由のひとつ**になっています。

### 3. 広告収入に頼らないマネタイズポイントを用意できている

広告から得られる収入は本当に微々たるものです。まとまった金額を得るためには、相当なユーザー数が必要になります。

「王道ジャンルのニッチを攻める」という戦い方を前提にするなら、広告以外の収益ポイントは必ず用意するべきと思いました。

そして、「広告以外の収益ポイントを用意する」という点においても、Webアプリよりネイティブアプリの方が有利そうだ、という気づきも得られました。
...
これを正しいとするならば、やはりWebアプリよりもネイティブアプリの方が収益化面で有利と言えそうです。
